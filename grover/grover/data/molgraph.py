...
class MolCollator(object):
    ...
    def __call__(self, batch):
        ...
        prot_enc_batch = [torch.tensor(d.protein_encoded) for d in batch]
        prot_seq_lens = [len(seq) for seq in prot_enc_batch]
        
        batch_graph = mol2graph(smiles_batch, self.shared_dict, self.args)
        graph_comp = batch_graph.get_components()
        mask = torch.Tensor([[x is not None for x in tb] for tb in target_batch])
        targets = torch.Tensor([[0 if x is None else x for x in tb] for tb in target_batch])
        return smiles_batch, graph_comp, features_batch, mask, targets, prot_enc_batch, prot_seq_lens
