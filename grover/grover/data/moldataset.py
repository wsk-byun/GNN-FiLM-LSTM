...
class MoleculeDatapoint:
    ...
    def __init__(self,
        ...
        # Create targets
        # The original makes everything after SMILES column as targets
        # self.targets = [float(x) if x != '' else None for x in line[1:]]
        self.targets = [float(line[1])] if line[1] != '' else [None]

        if len(line) >= 3 and line[2] != '':
            self.protein_sequence: str = line[2]
        else:
            self.protein_sequence = None

        if self.protein_sequence is not None:
            idxs = [
                AA_TO_IDX.get(residue, UNK_AA_IDX)
                for residue in self.protein_sequence
            ]
            self.protein_encoded = np.array(idxs, dtype=np.int64)
        else:
            self.protein_encoded = None
...
class MoleculeDataset(Dataset):
    ...
    def protein_sequence(self) -> List[str]:
        """
        Returns protein sequence strings associated with the molecules.

        :return: A list of protein sequences.
        """
        return [d.protein_sequence for d in self.data]

    def protein_encoded(self):
        return [d.protein_encoded for d in self.data]
